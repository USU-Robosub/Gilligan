...
int main(int argc, char **argv) {
	int fd[3];
	float buf[3];
	std::string filename[3] = {"/dev/ttyUSB0",
		                       "/dev/ttyUSB1",
							   "/dev/ttyUSB2"};
...
//	}
	...
	for(int i = 0; i < 3; i++) {
		fd[i] = open(filename[i].c_str(), O_RDWR | O_NOCTTY | O_NDELAY);

		if (fd[i] == -1) {
			printf("System failed to open %s: %s(%d)\n", filename[i].c_str(), strerror(errno), errno);
			return -1;
		}
	}
	...
	while (ros::ok()) {
		printf("loop\n");
		std::string line = getTTYLine(fd[0]);
		if (line.length() > 0) {
